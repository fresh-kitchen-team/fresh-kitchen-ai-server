...
process_receipt_raw() + filter_with_gemini() 예측과 비교한다.
품목 Precision/Recall/F1, 카테고리 정확도, 구매일(purchasedAt) 정확 일치율을 낸다.
...
def main():
    ...
    print(f"🧾 영수증 OCR 평가 시작 — 샘플 {len(samples)}장\n")
    ...
    for img_path, gt in samples:
        ...
        s = score_items(result.get("ingredients", []), gt.get("ingredients", []))
        tot_tp += s["tp"]; tot_fp += s["fp"]; tot_fn += s["fn"]; tot_cat += s["cat_correct"]
        p, r, f1 = prf(s["tp"], s["fp"], s["fn"])

        # 구매일: 정확 일치 (null == null 도 정답으로 인정)
        ...
        rows.append({
            "image": name,
            "date_ok": int(date_ok),
            "pred_date": pred_date if pred_date is not None else "",
            "gt_date": gt_date if gt_date is not None else "",
        ...
        print(f"📷 {name}")
        print(f"   📅 구매일: 예측 {pred_date} / 정답 {gt_date}  {'✅' if date_ok else '❌'}")
    ...
    print(f"   구매일 정확 일치율 : {date_acc:5.1f}% ({date_correct}/{len(samples)})")
    print(f"   품목 Precision     : {P*100:5.1f}%")
    print(f"   품목 Recall        : {R*100:5.1f}%")
    print(f"   품목 F1            : {F1*100:5.1f}%")
    ...
    fieldnames = ["image", "date_ok", "pred_date", "gt_date",
