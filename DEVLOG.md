...
| **6.14** | 생성형 파이프라인 정량 평가 | 영수증·냉장고 정답셋 기반 P·R·F1 평가 도구 구축 + 측정으로 냉장고 포괄어 억제 프롬프트 개선 |
...
세 파이프라인 모두 정확도 척도를 갖췄다. 분류 모델은 닫힌 70클래스라 **클래스별 정확도 + 혼동행렬**로(`test_V2_M.py`·`eval_val_V2_M.py`), 영수증·냉장고는 출력이 **"품목 리스트 + 카테고리(+구매일)"** 라 정답 JSON 대비 **집합 P·R·F1** 로 채점한다(`eval_common.py` + `eval_receipt.py`·`eval_fridge.py`).

| 파이프라인 | 평가 방식 | 핵심 지표 |
|---|---|---|
| 분류 | test셋 클래스별 정확도 + val 혼동행렬 | val_acc **96.60%**, Macro F1 96.6% (70클래스) |
| 영수증 OCR | 정답 JSON 대비 집합 P·R·F1 + 구매일 | 구매일 100%, 품목 F1 0.77, 카테고리 89.5% |
| 냉장고 감지 | 정답 JSON 대비 집합 P·R·F1 | 품목 F1 0.59, 카테고리 96% |

> 영수증·냉장고는 1차 소규모 측정(각 4·5장, 표기 차이 보정 후).

**어떻게 읽었나**
- **분류** — val_acc 96.60%로 견고. 남은 오류는 시각적으로 유사한 쌍(치즈↔버터, 감자↔고구마 등)에 집중 → 무작위가 아니라 유사 클래스 데이터의 문제 (상세는 "모델 진화 과정").
- **영수증** — 낮아 보이는 품목 F1 의 주원인은 모델이 아니라 **채점·정답지**였다. `포도봉봉`/`포도음료`처럼 같은 물건의 표기 차이가 FN·FP 로 이중 감점됨(보정 시 0.56→0.77). 정답지가 틀리고 모델이 맞은 사례까지 있었음.
- **냉장고** — 결국 **잘 보이는 식재료는 정확히 잡고, 가려져서 안 보이는 건 못 잡았다.** → 모델 인식력 문제라기보다 **"보이느냐"의 문제**.

**어느 방향으로 갈까**
- **분류** — 혼동 쌍 타깃 hard negative 보강 (grid search 아님).
- **영수증** — 표기 차이 감점을 매칭 자동화로 제거하고, 정답지의 추측 표기를 정제.
- **냉장고** — 가림 누락은 사용자 확인 UX·촬영 가이드로 흡수, 모델 쪽은 포괄어·헛검출만 프롬프트로 관리(완료, FP 29→20).
- **공통** — 영수증·냉장고는 표본이 작아(4·5장) 절대 수치 비교는 각 15~20장 확보 후 유효.
...
**효과(측정)**: 한 풀샷의 오검출 품목이 11개(대부분 포괄어) → 2개(구체 품목)로 줄며 **포괄어 출력이 사실상 사라짐**(전체 FP 29 → 20). 재측정 시 정답지도 함께 보강돼 precision 절대치 비교는 제한적이지만, 포괄어 소멸은 프롬프트 효과가 명확. Recall 도 거의 유지돼 우려한 "제외 강화로 인한 누락 폭증"은 없었다. *제외를 강화하는 프롬프트는 precision↑·recall↓ 양날의 검이므로 변경 시 반드시 재측정해 확인.*
...
- **Gemini 파이프라인(냉장고·OCR) 평가 — 1차 구축, 표본 확대가 과제** — 정답 라벨셋이 없어 정성 확인에 그쳤던 문제를, 목록 추출 task 에 맞게 정답 JSON 대비 **Precision(오탐)·Recall(누락)·카테고리** 를 분리 측정하는 도구로 해소(상세는 "정확도 평가"). 재고 앱 특성상 오탐이 신뢰에 더 치명적이라 프롬프트를 보수적(불확실하면 제외)으로 설계했고, 프롬프트 강화로 포괄어 오검출이 실제 줄어듦을 측정으로 확인. 다만 표본이 각 4·5장으로 작아 **평가셋 확대(각 15~20장)** 가 남은 과제.
