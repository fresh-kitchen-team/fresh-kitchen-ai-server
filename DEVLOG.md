...
6. [모델 진화 과정](#모델-진화-과정)
7. [정확도 평가 — 영수증 OCR · 냉장고 감지](#정확도-평가--영수증-ocr--냉장고-감지)
...
## 정확도 평가 — 영수증 OCR · 냉장고 감지

분류 모델과 달리 두 파이프라인은 출력이 **"품목 리스트 + 카테고리(+영수증은 구매일)"** 라 단순 정확도로 잴 수 없어, 정답 JSON과 예측을 **집합 기반 P·R·F1** 로 채점하는 도구를 별도로 만들었다 (`models/eval_common.py` + `eval_fridge.py`·`eval_receipt.py`).

**1차 측정 (소규모: 냉장고 5장 / 영수증 4장, 표기 차이 보정 후)**

| 파이프라인 | 구매일 정확도 | 품목 F1 | 카테고리 정확도 |
|---|---|---|---|
| 영수증 OCR | 4/4 (100%) | 0.77 | 89.5% |
| 냉장고 감지 | — | 0.59 | 96% |

**지표 분석**
- **구매일·카테고리는 이미 견고** — 구매일 100%(날짜 미표기 시 `null` 까지 정확), 카테고리는 이름만 맞히면 거의 맞음(병목 아님).
- **품목 F1 을 낮춘 건 모델보다 채점·정답지 문제** — `포도봉봉`/`포도음료`처럼 같은 물건의 표기 차이가 FN·FP 로 이중 감점됨(보정 시 영수증 0.56→0.77). GS25 영수증은 정답지가 틀리고 모델이 맞은 사례도 있었음.
- **냉장고에 남는 진짜 누락은 가림** — 서랍·적재로 안 보이는 품목에 집중. 과밀 풀샷(fridge1)은 정답지 자체가 불완전.

**더 필요한 것**
- **평가셋 확대** (각 15~20장) — 표본 5/4 로는 한 장의 영향이 커 절대 수치 비교 불가.
...
**효과(측정)**: 한 풀샷의 오검출 품목이 11개(대부분 포괄어) → 2개(구체 품목)로 감소, 전체 오검출(FP) 29 → 20, Precision 0.41 → 0.61(보정 기준). Recall 은 거의 유지 → 우려한 "제외 강화로 인한 누락 폭증"은 없었다. *제외를 강화하는 프롬프트는 precision↑·recall↓ 양날의 검이므로 변경 시 반드시 재측정해 확인.*
