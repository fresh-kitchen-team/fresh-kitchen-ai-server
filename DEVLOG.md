...
| **6.06** | 안정화 심화 / 분류 평가 고도화 | 엔드포인트 동기 전환(이벤트 루프 블로킹 회피) + data_split 멱등성·손상 이미지 예외 처리 보강 + 학습·평가 로그 버전별 폴더 재구성 + 분류 혼동행렬(P·R·F1·Macro) 평가 추가 |
...
### 분류 — 요약
val_acc 96.60%로 견고. 남은 오류는 시각적으로 유사한 쌍(치즈↔버터, 감자↔고구마 등)에 집중 → 무작위가 아니라 유사 클래스 데이터의 문제 (상세는 "모델 진화 과정").
