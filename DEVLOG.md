...
> 한 장의 사진으로 **음식 분류 · 영수증 OCR · 냉장고 인벤토리 자동 구축** 을 제공하는 3종 ML 파이프라인을 FastAPI 단일 서버에 통합함.
...
6. [모델 진화 과정](#모델-진화-과정)
7. [정확도 평가 — 분류 · 영수증 · 냉장고](#정확도-평가--분류--영수증--냉장고)
8. [마주친 문제와 해결](#마주친-문제와-해결)
9. [데이터 파이프라인](#데이터-파이프라인)
10. [회고 및 배운 점](#회고-및-배운-점)
11. [기술 스택](#기술-스택)
12. [링크](#링크)
...
| **기간** | 2026.03 ~ 2026.06 (4개월, 산학협력 프로젝트) |
...
> 위 다이어그램의 엔드포인트는 가독성을 위해 축약, 실제 경로에는 `/internal/v1` 프리픽스가 붙음 (예: `POST /internal/v1/food-classification`).
...
> 주차 기준: 1주차 3/3~3/9, 이후 7일 단위.

| 주차 | 시기 | 단계 | 내용 |
|---|---|---|---|
| **1주차** | 3월 초 | 데이터셋 구축 착수 | 공개 데이터셋 검토 — 한국 식재료 클래스 자체가 없거나 라벨 오류·무관 이미지 혼입으로 학습 부적합 판정. DuckDuckGo 크롤링으로 직접 수집 시작. 32가지 한국 식재료 클래스 정의. |
| **2주차** | 3.16 | 분류기 착수 | EfficientNet 백본 전체 freeze → head만 학습하는 전이학습 구조로 첫 분류기 구성 시작. 초기 데이터 200~500장 규모. |
| **5주차** | 4.02 | 전처리·증강 정비 | 크롤링 이미지(정면·단독·고화질)와 실제 스마트폰 사진(기울어짐·흐림·조명 편차) 간 분포 차이 문제 인식. `RandomPerspective`·`GaussianBlur`·`ColorJitter`·`RandomAdjustSharpness`·`RandomRotation` 5종 증강 추가해 실사 환경 시뮬레이션. |
| **5주차** | 4.03 | 1차 자문 피드백 | 현재 구조(EfficientNet 분류 + YOLO 냉장고) 검토. 32클래스 E0에서 ~98% 확인. 클래스 확장 고려해 V2-M 변경 권고, 냉장고는 YOLOv8 계획, 유통기한은 UI 입력으로 결정. |
| **5주차** | 4.05 | 백본 교체 | 32-class EfficientNet-E0 → **V2-M** 전환. 같은 EfficientNet 계열이지만 V2는 학습 안정성·표현력이 한 단계 위 — 35→60→70 클래스 확장 시 val_acc 94~96% 유지의 기반이 됨. |
| **7주차** | 4.16 | 영수증 OCR 2단계 구현 | 1단계: Google Document AI로 `line_item` 엔티티 추출 후 정규식 `[^가-힣\s]`으로 한글 필터링. 2단계: 추출 텍스트를 Gemini API에 전달, `response_mime_type="application/json"` JSON 강제로 `purchasedAt`(YYYY-MM-DD)·`ingredients[]` 반환. 정규식으로 날짜 형식 재검증 추가. |
| **8주차** | 4.23 | 파인튜닝 고도화 | 클래스별 가중치 저장 + backbone 상위 4개 블록(`features[-4:]`)과 head만 해동하는 선택적 파인튜닝 구조 도입. head-only 대비 도메인 특화 표현력 향상. |
| **8주차** | 4.24 | 2차 자문 피드백 | "그냥 VLM 쓰는 건 어때?" 피드백 수령. 이를 계기로 ① 분류 → ② 저신뢰(80% 미만) 시 VLM 위임 → ③ Gemini 라벨 `auto_labeled/` 자동 누적 → ④ 다음 학습 반영의 **self-improving 4단계 구조 직접 설계**. |
| **8주차** | 4.27 | VLM 폴백 구현 | 소프트맥스 최댓값 80% 미만 시 Gemini 2.5 Flash Vision으로 자동 위임. Gemini는 전체 클래스 목록을 프롬프트로 받아 목록 내 클래스명 또는 자유 한국어 라벨 반환. 폴백 이미지는 `dataset/auto_labeled/{label}/`에 자동 저장 — 운영 중 학습 데이터 자가 누적 구조 완성. |
| **9주차** | 5.02~ | Git 형상관리 시작 / 3파이프라인 통합 | 서버·파이프라인 코드 형상관리 시작. ver2(35클래스) 분류기를 FastAPI에 통합, 영수증 OCR·냉장고 감지까지 3종 파이프라인을 단일 서버에 통합. 백엔드와 JSON 응답 형식 협의·통일. |
| **9주차** | 5.03 | ver2 학습 | 35 클래스 첫 분류기 학습 완료. val_acc **94.56%** (영문 라벨, train 7,315 / val 1,803장). |
| **11주차** | 5.13 | ver3 학습 / 냉장고 파이프라인 전환 | 60 클래스로 확장 재학습 → val_acc 94.73%. 냉장고 감지: ① COCO 한국 식재료 미포함 ② YOLO+EfficientNet 연계안 Gemini 다중 호출 지연 ③ YOLO-World 한국어 불안정, 세 가지 이유로 **Gemini 2.5 Flash Vision 단독 전환** 확정. |
| **11주차** | 5.14 | 영수증 응답 구조 팀 조율 | 백엔드 팀과 응답 형식 협의 → 구매일(`purchasedAt`) + 식재료 목록(`ingredients[]`)만 반환하는 최소 형식으로 최종 확정. |
| **13주차** | 5.27 | ver4 학습 | 70 한글 클래스 + **Progressive Unfreezing** 도입. Epoch 5까지 `features[-4:]`+head 학습(LR=1e-4), Epoch 5부터 backbone 전체 해동(LR=1e-5). val_acc 95.42% (epoch 7 최고점, patience=5 소진으로 epoch 12 조기 종료 — best 모델 기준 backbone이 단 2에폭만 학습된 상태). |
| **13주차** | 5.28 | ver5 학습 ⭐ | ver4 로그(`docs/logs/classifier/ver4_0527/training_log.csv`) 분석으로 backbone 학습 부족 확인 → patience 5→8, epoch 30→50 완화해 재학습. val_acc **96.60%** (+1.18%p). epoch 29 최고점, ReduceLROnPlateau가 LR을 자동 반감(1e-5→…→6.25e-7)하며 epoch 37 조기 종료. |
| **13주차** | 5.29 | 운영 안정화 | ThreadPoolExecutor 방식의 타임아웃을 SDK 네이티브 `HttpOptions(timeout=...)` 로 교체(타임아웃 시 HTTP 요청 자체 중단, 워커 점유 없음). Bearer 토큰 + `hmac.compare_digest()` 타이밍 공격 방어, magic bytes 이중 검증 추가. ngrok 외부 접속 정비. |
| **14주차** | 6.06 | 동기 엔드포인트 전환 / 분류 평가 고도화 | `async def` → `def` 전환 — 블로킹 작업(torch 추론·Gemini 호출)이 이벤트 루프 점유 없이 FastAPI 외부 스레드풀에서 처리. `threading.Lock`으로 MPS 멀티스레드 추론 직렬화(OCR·냉장고는 네트워크 I/O라 잠금 없이 병렬 유지). 학습·평가 로그 버전별 폴더 재구성. `eval_val_V2_M.py` 구현으로 클래스별 Precision·Recall·F1 + 혼동행렬 분석 추가 — Macro P 96.8% / R 96.6% / F1 96.6% 산출. |
| **15주차** | 6.11 | LR 조정 실험 / ver5 최종 확정 | LR 1e-4 → 5e-5로 낮춰 재학습. 치즈↔버터, 돼지고기↔소고기·닭고기 등 **동일 클래스 쌍에서 동일 혼동 반복** 확인. 하이퍼파라미터가 원인이 아님 — 학습 이미지 자체의 시각적 유사성이 근본 원인. ver5를 최종 모델로 확정. |
| **15주차** | 6.14 | 생성형 파이프라인 평가 도구 구축 | `eval_common.py`·`eval_receipt.py`·`eval_fridge.py` 정답셋 평가 도구 직접 구현. 냉장고 포괄어 억제: 블랙리스트 방식 한계 확인(목록에 없는 포괄어 통과) → "구체 품목명이 아닌 모든 카테고리성 단어는 절대 쓰지 말 것" 원칙 기반 프롬프트로 전환, 포괄어 오검출 감소 확인. |
| **15주차** | 6.15 | 평가 방식 전환 / 오류 원인 진단 | 단일 F1 점수 → **'예측 vs 정답' 비교표** 방식 전환(표기 차이와 실제 오인식 분리). 영수증: 구매일 4/4, 불일치 대부분이 상품명↔일반명 표기 차이(의미상 정확). 냉장고: 가시 품목 안정적, 가림 누락(서랍·불투명 용기)·과밀 과검출 패턴 진단 완료. |
| **16주차** | 6.18 | 최종 발표 | — |
...
**해결**: 학습 단계만 **집의 Windows 데스크탑(NVIDIA GPU)** 으로 분리. `#7` 의 자동 감지 로직 덕분에 동일 코드를 그대로 실행 → CUDA 환경에서 AMP 자동 활성화. 학습 산출물(`.pth`, `docs/logs/classifier/<버전>/`) 만 운영 서버로 옮겨 반영.
...
| **ver4** (5/27) | 70 (한글) | Train 12,714 / Val 3,147 | 30 | 5 | 95.42% | 한글 클래스명 전환 + Progressive Unfreezing 도입 |
...
**로그로 확인한 원인** (`docs/logs/classifier/ver4_0527/training_log.csv`): ver4 는 backbone 전체 해동이 epoch 6 에 일어났고 epoch 7 에 바로 최고점(95.42%) 달성 → **저장된 best 모델 기준 backbone은 단 2 에폭(epoch 6·7)만 학습된 상태.** patience=5 소진으로 epoch 12 에 조기 종료되지만, 저장된 가중치는 epoch 7 것이라 backbone이 충분히 수렴하지 못한 게 찜찜함의 정체.
...
검증셋 기준으로 단순 정확도를 넘어 **클래스별 Precision·Recall·F1**(`models/food_classifier/eval_val_V2_M.py`, 결과 `docs/logs/classifier/ver5_0528/confusion_matrix_val.csv`·`class_metrics_val.csv`)까지 산출함. **Macro 평균 P 96.8% / R 96.6% / F1 96.6%** 로 특정 방향 편향 없이 고르게 높았으며, 혼동행렬로 클래스별 오분류 방향을 분석함.

- **치즈 = 최대 단일 혼동**: 치즈 오류의 약 40%(4/10)가 **버터**로 분류됨 — 둘 다 옅은 노란 블록 형태. "치즈가 약하다"를 넘어 **원인이 버터와의 혼동임을 특정**.
- **생고기 클러스터**: 돼지고기 오류 대부분이 소고기·닭고기(각 3건). 색·질감이 유사해 분류가 어려운 케이스.
...
분류 모델은 닫힌 70클래스라 **클래스별 정확도 + 혼동행렬**(`test_V2_M.py`·`eval_val_V2_M.py`)로 정량 평가함. 반면 영수증·냉장고는 출력이 자유 표기의 품목 리스트라 단일 점수(F1)가 표기 차이에 휘둘려 오히려 오해를 부름 — 그래서 점수 대신 **사진별 '예측 vs 정답' 비교표**(`eval_common.py` + `eval_receipt.py`·`eval_fridge.py`)로 "실제로 뭐가 있었고 모델은 뭐라 뽑았나"를 직접 봄. 품목 이름은 동의어 보정 없는 정확 일치.

| 파이프라인 | 평가 방식 | 결과 |
|---|---|---|
| 분류 | test셋 클래스별 정확도 + val 혼동행렬 | val_acc **96.60%**, Macro F1 96.6% (70클래스) |
| 영수증 OCR | 사진별 예측 vs 정답 비교표 + 구매일 | 구매일 **4/4**, 품목 대부분 추출(차이는 주로 표기) |
| 냉장고 감지 | 사진별 예측 vs 정답 비교표 | 보이는 건 잡고 가려진 건 누락, 과밀 사진은 과검출 경향 |

> 영수증·냉장고는 1차 소규모(각 4·5장).

### 분류 — 오류 원인 진단

val_acc 96.60%, Macro F1 96.6%로 전반적으로 견고함. 70개 클래스 중 24개는 100% 정확도를 기록했고, 오류도 무작위가 아닌 **시각적으로 유사한 쌍에 집중**되어 원인을 특정할 수 있었음. 96% 벽을 넘기 위해 혼동행렬(`eval_val_V2_M.py`, `docs/logs/classifier/ver5_0528/`)로 남은 오류의 정체를 분석함.

- **오류가 무작위가 아님**: 치즈↔버터(옅은 노란 블록), 돼지고기↔소고기·닭고기(날고기 색·질감), 고추장↔된장(장류 용기), 감자↔고구마(구근 형태) 등 **시각적으로 유사한 쌍에 집중**.
- **하이퍼파라미터 추가 조정 시도**: 혼동 패턴을 확인 후 learning rate·patience 등을 조정해 추가 실험을 해봤으나, **같은 클래스 쌍에서 동일한 혼동이 반복**됐다.
- **결론 — 데이터 문제**: 오류가 하이퍼파라미터가 아니라 **학습 이미지 자체가 시각적으로 너무 유사한 것**이 원인. 날고기끼리, 소스류끼리는 사람도 사진만으론 헷갈리는 케이스다. 추가 튜닝보다 **혼동 쌍 타깃 hard negative 보강**이 정공법이라는 결론으로, ver5를 최종 모델로 확정.

### 영수증 OCR — 오류 원인 진단

구매일 추출은 4/4 완벽했고, 식품 항목도 대부분 추출됐음. `예측 vs 정답` 비교표상 **불일치의 정체는 대부분 '표기 차이'**였음 — 모델이 영수증의 상품명을 그대로/약하게 일반화해 뽑는 것.

- **구매일은 견고**: 4/4. 날짜 미표기 영수증(피코크)에도 `null` 을 정확히 반환.
- **차이의 정체 = 브랜드·수식어**: `마늘빅프랑크`/`소시지`, `초마짬뽕`/`짬뽕`, `목심`/`돼지고기`, `칵테일새우`/`새우`, `깐마늘`/`마늘`, `생맥주`/`맥주`. GS25 영수증은 3품목 전부가 상품명↔일반명 차이라 표면상 0% 일치지만, **의미적으론 모두 정확**.
- **진짜 누락은 드묾**: 실제 품목인데 추출 자체가 안 된 건 `적양배추`·`양상추`(receipt1) 정도. → 인식이 아니라 **표기 정규화가 과제**.

### 냉장고 감지 — 오류 원인 진단

계란·콜라·소주·마요네즈처럼 눈에 잘 보이는 품목은 안정적으로 인식했고, 카테고리 분류도 전반적으로 정확했음. 오류는 세 가지 유형으로 뚜렷이 갈림.

- **① 표기 차이 (모델 자체 명명)**: `파마산치즈`/`치즈`, `참소스`/`고기양념`, `다진마늘`/`마늘`, `식용유`/`들기름`. 영수증과 같은 결.
- **② 가림 누락 — 가장 일관된 약점**: `계란`(계란 가드)·`옥수수`·`당근`·`김치`(서랍)·`두부`·`마가린`(불투명 용기)·`굴소스`(캔)가 반복적으로 놓침. 서랍·용기·적재로 **물리적으로 안 보이는 것**에 집중 → 모델 인식력이 아니라 "보이느냐"의 문제.
- **③ 과밀 사진의 과검출/정답지 미달**: 가장 빽빽한 fridge1·3·4 에서 모델이 정답지에 없는 실제 조미료(`간장`·`식초`·`돈까스소스`)를 다수 출력. 모델 과검출인지 정답지가 덜 채워진 건지 **사진 재확인이 필요한 영역**이며, 일부 포괄어(`드레싱`·`물`)도 잔존.

→ 냉장고의 본질은 **"잘 보이면 잡고, 가려지면 놓치며, 과밀 사진은 사람 재확인이 필요"**. 분류 모델과 달리 Gemini는 재학습이 불가하므로, 개선 레버는 모델이 아니라 **촬영 가이드·사용자 확인 UX·정답지 정제**임.

### 어느 방향으로 갈까
- **분류** — 혼동 쌍 타깃 hard negative 보강 (grid search 아님).
- **영수증·냉장고** — 모델이 상품명을 **일반 품목명으로 정규화**하도록 유도(표기 일관성) — 실제 재고 중복 방지에도 직결.
- **냉장고** — 가림 누락은 사용자 확인 UX·촬영 가이드로 흡수, 과밀 풀샷은 정답지를 전수로 채우거나 평가셋에서 제외.
- **공통** — 표본이 작아(각 4·5장) 절대 판단보다 경향·개선 방향 도출 용도. 각 15~20장 확보 후 재확인.
...
**교훈**: 임계값 하향보다 **모델 자체 개선(ver5)이 더 맞는 방향**. 클래스 수 증가 시 백본 학습량 확보가 필수.
...
**1차 해결(블랙리스트)**: 프롬프트에 금지 단어 예시를 나열.
> "즉석식품", "음식", "채소", "소스", "식품" 처럼 포괄적인 단어는 쓰지 말고...

**한계 — 평가로 발견**: 정확도 측정에서 모델이 금지 목록에 **없던** 다른 포괄어(`신선채소`·`음료수`·`주스`·`견과류`·`설탕` 등)를 그대로 출력. 예시 나열을 **단어 블랙리스트**로 좁게 해석한 것. 특히 과밀·흐릿한 풀샷에서 개별 식별이 안 되니 카테고리성 단어로 얼버무리는 경향.

**2차 해결(원칙 기반)**: 블랙리스트를 **원칙 + 강한 제외**로 전환.
> 상위 분류명·포괄어(…구체적 품목명이 아닌 모든 카테고리성 단어)는 절대 쓰지 마. … 정확히 단정할 수 없으면 추측하지 말고 그 항목을 반드시 빼.

**효과**: 카테고리성 단어(`채소`·`음료수`·`주스` 등) 출력이 눈에 띄게 줄고 구체 품목명 위주로 바뀌었다. 다만 `드레싱`·`물` 같은 일부 일반어는 여전히 남아 완전히 사라지진 않았다. *제외를 강화하는 프롬프트는 precision↑·recall↓ 양날의 검이므로 변경 시 반드시 재측정해 확인.*
...
공개 데이터셋 한계로 직접 크롤링·선별해 약 **15,000장**(ver5 기준 train 12,714 / val 3,145)의 자체 데이터셋을 구축함 (배경은 "마주친 문제" 1번 참고). 새 클래스 추가 시 5단계 자동 파이프라인:
...
- **Gemini 파이프라인(냉장고·OCR) 평가 — 1차 구축, 표본 확대가 과제** — 정답 라벨셋이 없어 정성 확인에 그쳤던 문제를, 사진별 **'예측 vs 정답' 비교표**로 "실제 vs 모델 출력"을 대조하는 도구로 해소(상세는 "정확도 평가"). 재고 앱 특성상 오탐이 신뢰에 더 치명적이라 프롬프트를 보수적(불확실하면 제외)으로 설계했고, 프롬프트 강화로 포괄어 오검출이 실제 줄어듦을 확인. 다만 표본이 각 4·5장으로 작아 **평가셋 확대(각 15~20장)** 가 남은 과제.

### 배운 점

- **Early Stopping patience 는 모델 복잡도·클래스 수에 비례** 해야 함. ver4에서 patience=5 는 35→70 클래스 확장 후 너무 짧았음.
- **80% 신뢰도 임계값은 70 클래스 분류기의 "노이즈 영역" 경계** 였음. 그 미만은 차라리 외부 LLM에 위임하는 게 정확도·UX 모두에 유리.
- **외부 LLM은 결정적이지 않음** — JSON 강제(`response_mime_type`), 타임아웃, isinstance 가드, 정규식 재검증 같은 다중 안전장치 필수.
- **`hmac.compare_digest`, magic bytes 검증** 같은 보안 기본기를 직접 적용 — 타이밍 공격 방어(`==` 비교의 응답시간 누출 차단)·헤더 위조 방어(실제 바이트로 형식 검증).
- **학습 데이터와 실제 배포 환경의 분포 차이는 증강으로 좁힐 수 있음** — 크롤링 데이터(정면·고화질)로만 학습하면 스마트폰 실사 사진에서 성능이 떨어짐. 별도 실사 데이터 없이도 RandomPerspective·GaussianBlur 같은 증강 몇 줄로 갭을 상당히 줄일 수 있다는 걸 직접 확인함.
...
- **학습 로그 데이터**: `docs/logs/classifier/ver2_0503`~`ver5_0528/` (버전+날짜별 학습·평가 로그 전체 보존)
