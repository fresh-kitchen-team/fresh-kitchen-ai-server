...
검증셋 기준으로 단순 정확도를 넘어 **클래스별 Precision·Recall·F1**(`models/food_classifier/eval_val_V2_M.py`, 결과 `docs/logs/classifier/ver5_0528/confusion_matrix_val.csv`·`class_metrics_val.csv`)까지 산출했다. **Macro 평균 P 96.8% / R 96.6% / F1 96.6%** 로 특정 방향 편향 없이 고르게 높았으며, 혼동행렬로 클래스별 오분류 방향을 분석했다.
...
분류 모델은 닫힌 70클래스라 **클래스별 정확도 + 혼동행렬**(`test_V2_M.py`·`eval_val_V2_M.py`)로 정량 평가한다. 반면 영수증·냉장고는 출력이 자유 표기의 품목 리스트라 단일 점수(F1)가 표기 차이에 휘둘려 오히려 오해를 부른다 — 그래서 점수 대신 **사진별 '예측 vs 정답' 비교표**(`eval_common.py` + `eval_receipt.py`·`eval_fridge.py`)로 "실제로 뭐가 있었고 모델은 뭐라 뽑았나"를 직접 본다. 품목 이름은 동의어 보정 없는 정확 일치.
...
val_acc 96.60%, Macro F1 96.6%로 전반적으로 견고하다. 70개 클래스 중 24개는 100% 정확도를 기록했고, 오류도 무작위가 아닌 **시각적으로 유사한 쌍에 집중**되어 원인을 특정할 수 있었다. 96% 벽을 넘기 위해 혼동행렬(`eval_val_V2_M.py`, `docs/logs/classifier/ver5_0528/`)로 남은 오류의 정체를 분석했다.
...
구매일 추출은 4/4 완벽했고, 식품 항목도 대부분 추출됐다. `예측 vs 정답` 비교표상 **불일치의 정체는 대부분 '표기 차이'**였다 — 모델이 영수증의 상품명을 그대로/약하게 일반화해 뽑는 것.
...
계란·콜라·소주·마요네즈처럼 눈에 잘 보이는 품목은 안정적으로 인식했고, 카테고리 분류도 전반적으로 정확했다. 오류는 세 가지 유형으로 뚜렷이 갈린다.
...
→ 냉장고의 본질은 **"잘 보이면 잡고, 가려지면 놓치며, 과밀 사진은 사람 재확인이 필요"**. 분류 모델과 달리 Gemini는 재학습이 불가하므로, 개선 레버는 모델이 아니라 **촬영 가이드·사용자 확인 UX·정답지 정제**다.
