...
| 시기 | 단계 | 내용 |
|---|---|---|
| **3월** | 데이터셋 구축 | 공개 데이터셋은 원하는 한국 식재료·품질이 부족 → 크롤링 + 수작업 선별로 자체 데이터셋 수집 시작 |
...
| **4.03** | 1차 자문 피드백 | EfficientNet 단일 분류 + YOLO 냉장고 전체 인식 구도 검토. 32 클래스 E0에서 ~98%, "클래스 증가에 따른 V2-M" 변경 고려. 유통기한은 UI 입력, 냉장고는 YOLOv8 계획 |
...
| **4.24** | 2차 자문 피드백 | 평가용 reference dataset 필요성 제기 + "그냥 VLM 쓰는 건 어때?" 피드백. 이를 계기로 **VLM을 Auto-Labeler로 쓰는 Self-improving 4단계 구조 구현** |
...
| **5.14** | 영수증 응답 구조 팀 조율 | 백엔드 팀과 응답 형식 협의 → 구매일·식재료 목록만 반환하는 형식으로 최종 확정 |
...
| **6.06** | 안정화 심화 / 분류 평가 고도화 | 엔드포인트 동기 전환(이벤트 루프 블로킹 회피) + 학습·평가 로그 버전별 폴더 재구성 + 분류 혼동행렬(P·R·F1·Macro) 평가 추가 |
| **6.11** | 분류 모델 추가 실험 | LR 1e-4 → 5e-5 조정 후 재학습 — 성능 차이 미미. 혼동행렬 분석과 함께 원인이 하이퍼파라미터가 아닌 유사 클래스 데이터임을 확인 → ver5 최종 확정 |
| **6.14** | 생성형 파이프라인 평가 착수 | 영수증·냉장고 정답셋 평가 도구 구축 + 측정으로 냉장고 포괄어 억제 프롬프트 개선 |
| **6.15** | 평가 분석 | 단일 점수(F1) 대신 '예측 vs 정답' 비교표로 전환 + 영수증·냉장고 오류 원인 진단 |
