...
**어떻게 읽었나** (비교표 직접 관찰)
- **분류** — val_acc 96.60%로 견고. 남은 오류는 시각적으로 유사한 쌍(치즈↔버터, 감자↔고구마 등)에 집중 → 무작위가 아니라 유사 클래스 데이터의 문제 (상세는 "모델 진화 과정").
- **영수증** — 구매일은 4/4로 정확. 품목도 비교표상 대부분 추출되며, 정답과의 차이는 거의 **표기**다 — `가슴살`/`닭가슴살`, `짬뽕`/`초마짬뽕`, `곰탕`/`한우곰탕`, `깐마늘`/`마늘`처럼 같은 물건을 다르게 부른 것(특히 편의점 영수증은 브랜드 상품명을 그대로 뽑음). 인식 자체는 잘 된다.
- **냉장고** — 세 가지 양상이 섞인다. ① 표기 차이(`파마산치즈`/`치즈`), ② **서랍·적재로 가려진 품목(계란·당근·옥수수)은 실제로 놓침**(가장 빽빽한 사진에서 두드러짐), ③ 반대로 **예측이 정답보다 많은 사진**도 있어(모델 과검출이거나 과밀 냉장고의 정답지가 덜 채워진 것). → "잘 보이면 잡고, 가려지면 놓치며, 과밀 사진은 사람 재확인이 필요"가 핵심.

**어느 방향으로 갈까**
- **분류** — 혼동 쌍 타깃 hard negative 보강 (grid search 아님).
- **영수증·냉장고** — 모델이 상품명을 **일반 품목명으로 정규화**하도록 유도(표기 일관성) — 실제 재고 중복 방지에도 직결.
- **냉장고** — 가림 누락은 모델로 못 고치므로 사용자 확인 UX·촬영 가이드로 흡수. 과밀 풀샷은 정답지를 전수로 채우거나 평가셋에서 제외.
