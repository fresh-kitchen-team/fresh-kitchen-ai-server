...
### 2. 냉장고 감지 인식률 저하 — 해상도 조정

1024px 기준으로 Gemini에 전달하니 이미지 품질이 떨어져 식재료를 제대로 인식하지 못하는 경우 발생.

**해결**: 최대 해상도를 **2048px** 로 상향. 해상도가 올라가면 Gemini 처리 시간도 늘어날 수 있어 `GEMINI_TIMEOUT` 기본값도 30 → **60초**로 함께 상향. 이후 타임아웃 초과 사례는 발생하지 않음.
...
### 6. 데이터 증강 — 크롤링 이미지와 실제 스마트폰 사진의 분포 차이
...
**해결**: 학습 전처리에 스마트폰 촬영 조건을 시뮬레이션하는 증강 추가 (`training/train_EfficientNet_V2_M.py`).
