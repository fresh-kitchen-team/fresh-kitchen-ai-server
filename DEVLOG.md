...
| 주차 | 시기 | 내용 |
|:---:|:---:|---|
| 1주차 | 3월 초 | **데이터셋 구축 착수** — 공개 데이터셋 검토 결과 한국 식재료 클래스 자체가 없거나 라벨 오류·무관 이미지 혼입으로 학습 부적합 판정. DuckDuckGo 크롤링으로 직접 수집 시작. 32가지 한국 식재료 클래스 정의. |
...
| 5주차 | 4.05 | **백본 교체** — EfficientNet-E0 → V2-M 전환. 같은 EfficientNet 계열이지만 V2는 학습 안정성·표현력이 한 단계 위 — 클래스 60~70개 확장을 고려한 선제 대응. |
...
| 13주차 | 5.27 | **ver4 학습** — 70 한글 클래스 + Progressive Unfreezing 도입. Epoch 5까지 `features[-4:]`+head 학습(LR=1e-4), Epoch 5부터 backbone 전체 해동(LR=1e-5). val_acc 95.42% (epoch 7 최고점, patience=5 소진으로 epoch 12 조기 종료). |
...
| 13주차 | 5.29 | **운영 안정화** — 타임아웃을 SDK 네이티브 `HttpOptions(timeout=...)`로 교체(HTTP 요청 자체 중단, 워커 점유 없음). Bearer 토큰 + `hmac.compare_digest()` 타이밍 공격 방어, magic bytes 이중 검증 추가. ngrok 외부 접속 정비. |
| 14주차 | 6.06 | **동기 엔드포인트 전환 / 분류 평가 고도화** — `async def` → `def` 전환으로 블로킹 작업이 이벤트 루프 점유 없이 FastAPI 외부 스레드풀에서 처리. `threading.Lock`으로 MPS 멀티스레드 추론 직렬화. 학습·평가 로그 버전별 폴더 재구성. `eval_val_V2_M.py`로 클래스별 Precision·Recall·F1 + 혼동행렬 분석 추가 — Macro P 96.8% / R 96.6% / F1 96.6% 산출. |
...
| 15주차 | 6.14 | **생성형 파이프라인 평가 도구 구축** — `eval_common.py`·`eval_receipt.py`·`eval_fridge.py` 정답셋 평가 도구 직접 구현. 냉장고 포괄어 억제: 블랙리스트 방식 한계 확인 → "구체 품목명이 아닌 모든 카테고리성 단어는 절대 쓰지 말 것" 원칙 기반 프롬프트로 전환, 포괄어 오검출 감소 확인. |
