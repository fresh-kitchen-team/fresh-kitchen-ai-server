...
val_acc 96.60%, Macro F1 96.6%로 전반적으로 견고하다. 96% 벽을 넘기 위해 혼동행렬(`eval_val_V2_M.py`, `docs/logs/classifier/ver5_0528/`)로 남은 오류의 정체를 분석했다.
...
`예측 vs 정답` 비교표상 **불일치의 정체는 대부분 '표기 차이'**였다 — 모델이 영수증의 상품명을 그대로/약하게 일반화해 뽑는 것.
