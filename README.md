...
1. [한눈에 보는 시스템](#한눈에-보는-시스템)
2. [빠른 시작](#빠른-시작)
3. [환경 변수](#환경-변수)
4. [API 레퍼런스](#api-레퍼런스)
5. [시스템 동작 흐름](#시스템-동작-흐름)
6. [모델 학습 (Food Classifier)](#모델-학습-food-classifier)
7. [학습 이력](#학습-이력)
...
Gemini 2.5 Flash Vision 호출 (temperature=0, JSON 강제, 타임아웃 60초)
    ...
    │              ▼
    │          Python 레벨 후처리
    │           - name.strip() 기준 추가 중복 제거
    │           - normalize_category() 로 비표준 카테고리 → ETC 변환
    │              │
    │              ▼
    │          {"items": [...]} 반환
    │
    └─ 실패 / 타임아웃 ──► {"items": []} 반환 (재시도 안 함)
```
...
- 확신도 임계값 조정: `models/food_classifier/predict_V2_M.py:23` 의 `CONFIDENCE_THRESHOLD` 변경 (현재 80.0)
