...
| 파이프라인 | 기술 | 역할 |
|---|---|---|
| 음식 분류 | EfficientNet V2-M + Gemini 2.5 Flash (폴백) | 식재료 이미지를 70개 클래스로 분류 |
| 영수증 OCR | Google Document AI + Gemini 2.5 Flash | 영수증에서 구매일·식재료 목록 추출 |
| 냉장고 감지 | Gemini 2.5 Flash Vision | 냉장고 사진에서 식재료 목록 감지 |

**신뢰도 기반 자동 폴백**: 음식 분류 모델의 확신도가 80% 미만이면 Gemini Vision이 대신 판단하고, 분류 결과를 `dataset/auto_labeled/`에 자동 저장하여 이후 재학습에 활용합니다.
...
#### 응답 — EfficientNet이 직접 분류 (`confidence ≥ 80%`)
...
#### 응답 — Gemini 폴백 사용 (`confidence < 80%`, Gemini 성공)
...
#### 응답 — Gemini 폴백도 실패 (`confidence < 80%`, Gemini 실패)
...
- 전체 클래스 목록 및 카테고리 매핑은 `models/food_classifier/predict_V2_M.py`의 `CLASS_CATEGORY`를 참고하세요 (현재 70개 클래스, 한글 클래스명 사용)
...
EfficientNet V2-M 추론 (softmax × 100 → top-1 확신도)
    │
    ├─ confidence ≥ 80% ───────────────────────────────────────► 반환 (source: "efficientnet")
    │
    └─ confidence < 80%
            │
            ▼
...
#### 전처리 — 이미지 리사이즈 (`_resize_image`)

- 장변이 1024px 초과 시 비율 유지하며 축소 (`Image.LANCZOS`)
- JPEG 품질 85%로 재인코딩
- 전처리 후 MIME 타입은 항상 `image/jpeg`

#### Gemini 호출

- `temperature=0` 고정 → 동일 이미지에 대해 결정적(deterministic) 응답 보장
- `response_mime_type="application/json"` 강제 → `{"label": "클래스명", "category": "카테고리"}` 반환
- 70개 클래스 내 식재료는 클래스명 그대로 반환, 목록에 없는 식재료는 한국어 식품명으로 자유 반환 (예: `수박`, `망고`)
...
Gemini 2.5 Flash Vision API 호출로 처리합니다.

#### 전처리 — 이미지 리사이즈 (`_resize_image`)

API 호출 전 이미지를 전처리하여 전송 속도와 안정성을 높입니다.
...
- 타임아웃 시 1회 재시도, 2회 모두 실패 시 빈 배열 반환
...
- 브랜드명 제거 (예: "풀무원 두부" → "두부")
- 한글로만 출력
- 뚜껑이나 포장재로 내용물이 안 보이는 항목 제외
- 동의어 중복 제거 (예: "계란"과 "달걀" → "계란")
- `category` 분류 포함
- 냉장고가 비어 있을 경우 빈 배열 `[]` 반환 명시

#### 후처리

- Python 레벨에서 `name.strip()` 기준으로 추가 중복 제거
- `normalize_category()`로 유효하지 않은 카테고리 값을 `ETC`로 자동 변환
